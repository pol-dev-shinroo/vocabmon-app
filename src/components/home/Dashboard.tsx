...
export default function Dashboard() {
  ...
  const [atkCharges, setAtkCharges] = useState(10);
  ...
  useEffect(() => {
    ...
    // Clean up any existing global audio immediately
    if (globalDashboardAudio) {
      globalDashboardAudio.pause();
      globalDashboardAudio.src = '';
    }

    const audio = new Audio('/piano.mp3');
    audio.loop = true;
    audio.volume = 0.2;
    globalDashboardAudio = audio;
    ...
    const tryPlay = () => {
      ...
      if (playPromise !== undefined) {
        playPromise.then(() => {
          ...
          setIsMusicOn(false);
          interactionHandler = () => {
            audio.play().then(() => setIsMusicOn(true)).catch(e => {});
    ...
    return () => {
      if (globalDashboardAudio === audio) {
        audio.pause();
        audio.src = '';
        globalDashboardAudio = null;
      }
      if (interactionHandler) {
        document.removeEventListener('click', interactionHandler);
        document.removeEventListener('touchstart', interactionHandler);
  ...
  const toggleMusic = () => {
    if (!globalDashboardAudio) return;
    if (isMusicOn) {
      globalDashboardAudio.pause();
    } else {
      globalDashboardAudio.play().catch(e => console.log("Audio play blocked"));
    }
    setIsMusicOn(!isMusicOn);
  };
  
  const digimonNames = ["", "Digi-Egg", "Botamon", "Koromon", "Agumon", "Tyrannomon", "Greymon", "SkullGreymon", "MetalGreymon", "WarGreymon", "Omnimon"];
  const scheduleDays = ["Friday (Part 1)", "Friday (Part 2)", "Saturday (Part 1)", "Saturday (Part 2)", "Sunday (Morning)", "Monday (Part 1)", "Monday (Part 2)", "Tuesday (Part 1)", "Tuesday (Part 2)", "Wednesday"];
  const weeklyPlan = [ { day: "Friday", sets: [0, 1] }, { day: "Saturday", sets: [2, 3] }, { day: "Sunday", sets: [4], hasMidterm: true }, { day: "Monday", sets: [5, 6] }, { day: "Tuesday", sets: [7, 8] }, { day: "Wednesday", sets: [9], hasFinals: true } ];
  ...
  useEffect(() => {
    const initTimer = setTimeout(() => {
      ...
      if (localActiveWeekId !== activeWeekId) {
        const questKeys = ["current_word_set", "quest_spelling_done", "quest_exercise_done", "quest_test_done", "quest_midterm_done", "quest_final1_done", "quest_finale_done", "completedQuests"];
      ...
      setSpellingDone(localStorage.getItem("quest_spelling_done") === "true");
      setExerciseDone(localStorage.getItem("quest_exercise_done") === "true");
      setTestDone(localStorage.getItem("quest_test_done") === "true");
      setCurrentSet(parseInt(localStorage.getItem("current_word_set") || "0", 10));
      ...
      setIsLoaded(true);
      if (localStorage.getItem("trigger_fireworks") === "true") {
  ...
  useEffect(() => {
    if (isLoaded) {
      ...
      if (exp > lastExp) {
        ...
        localStorage.setItem("atk_charges", "10"); localStorage.setItem("game_attempts", "2");
      } else {
  ...
  useEffect(() => { if (isLoaded && activeQuestRef.current) setTimeout(() => { activeQuestRef.current?.scrollIntoView({ behavior: "smooth", block: "center" }); }, 300); }, [isLoaded]);

  const calculateLevelInfo = (totalExp: number) => {
    const rawLevel = Math.floor(totalExp / 150) + 1; const currentLevel = Math.min(rawLevel, 10); const expIntoCurrentLevel = totalExp % 150; const targetExpForLevel = 150;
    if (totalExp >= 1500) return { level: 10, currentExpInLevel: 150, targetExpForLevel: 150, percentage: 100 };
    return { level: currentLevel, currentExpInLevel: expIntoCurrentLevel, targetExpForLevel, percentage: (expIntoCurrentLevel / targetExpForLevel) * 100 };
  };
  ...
  const advanceToNextSet = () => {
    ...
    if (nextSet < totalSets) {
      ...
      localStorage.removeItem("quest_spelling_done"); localStorage.removeItem("quest_exercise_done"); localStorage.removeItem("quest_test_done");
      setCurrentSet(nextSet); setSpellingDone(false); setExerciseDone(false); setTestDone(false); triggerSilentSync();
      setTimeout(() => { activeQuestRef.current?.scrollIntoView({ behavior: "smooth", block: "center" }); }, 100);
    }
  };
    ...
    <div className="w-full max-w-md min-h-screen flex flex-col mx-auto bg-gray-50 relative animate-fade-in pb-16">
      ...
      {showFireworks && (
        <div className="fixed inset-0 pointer-events-none z-50 flex items-center justify-center overflow-hidden">
          {[...Array(12)].map((_, i) => (
            <div key={i} className="fw-particle" style={{ "--tx": `${Math.cos(i * 30 * (Math.PI / 180)) * 150}px`, "--ty": `${Math.sin(i * 30 * (Math.PI / 180)) * 150}px` } as React.CSSProperties}>
              {i % 2 === 0 ? "🌟" : "🎉"}
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm p-6 border border-gray-100 relative">
          <div className="flex justify-between items-start mb-6">
            ...
            <div className="flex gap-2">
              <Link href="/game" className="bg-emerald-50 hover:bg-emerald-100 text-emerald-600 border border-emerald-200 font-bold p-3 rounded-2xl shadow-sm transition-all transform hover:scale-105 flex items-center justify-center" title="Play Jump Game"><span className="text-xl">🎮</span></Link>
              <Link href="/collection" className="bg-amber-50 hover:bg-amber-100 text-amber-600 border border-amber-200 font-bold p-3 rounded-2xl shadow-sm transition-all transform hover:scale-105 flex items-center justify-center" title="View Hall of Fame"><span className="text-xl">🏆</span></Link>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-b from-blue-50 to-indigo-50 rounded-2xl pt-8 pb-4 px-6 flex flex-col items-center mb-6 border border-indigo-100/50 relative">
            <div className="absolute top-4 left-1/2 transform -translate-x-1/2 bg-white/90 backdrop-blur-sm border-2 border-indigo-100 text-indigo-800 font-black px-4 py-1 rounded-full text-xs uppercase tracking-widest shadow-sm z-10">{digimonNames[levelStats.level] || "Mystery"}</div>
          ...
          <div>
            <div className="flex justify-between text-sm font-bold text-gray-500 mb-2">
              <span>{levelStats.level === 10 ? "MAX LEVEL" : `EXP to Level ${levelStats.level + 1}`}</span>
              <span>{levelStats.currentExpInLevel} / {levelStats.targetExpForLevel}</span>
            </div>
            <div className="w-full bg-gray-100 rounded-full h-4 overflow-hidden relative"><div className="bg-indigo-500 h-full rounded-full transition-all duration-1000 ease-out" style={{ width: `${levelStats.percentage}%` }}></div></div>
          </div>
        </div>
      </div>
      ...
      {showEnergyAlert && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 animate-fade-in p-4"><div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center border-4 border-red-400 shadow-2xl"><div className="text-5xl mb-4 animate-bounce">💤</div><h3 className="text-2xl font-black text-gray-800 mb-2">Out of Energy!</h3><p className="text-gray-500 font-medium mb-8 leading-relaxed">Your partner needs to rest. Complete a quest to gain EXP and recharge their energy!</p><button onClick={() => setShowEnergyAlert(false)} className="w-full bg-red-500 hover:bg-red-600 text-white text-lg font-black py-4 rounded-xl shadow-md active:scale-95 transition-all">I&apos;ll go train! 🏃</button></div></div>
      )}

      {/* DEV EVOLUTION PREVIEW MODAL */}
      {devEvoModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-fade-in p-4">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center border-4 border-purple-500 shadow-2xl relative">
            <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 text-6xl animate-bounce">✨</div>
            <h3 className="text-3xl font-black text-purple-600 mb-2 uppercase italic tracking-wider mt-4">Evolution Preview!</h3>
            <p className="text-gray-600 font-bold mb-6 leading-relaxed">
              Dev Test: You evolved<br/>
              <span className="text-indigo-400 text-lg">{digimonNames[devEvoModal.from]}</span><br/>
              into <span className="text-purple-600 text-2xl font-black">{digimonNames[devEvoModal.to]}</span>!
            </p>
            <div className="bg-gradient-to-b from-purple-50 to-indigo-50 rounded-2xl pt-8 pb-4 px-6 flex flex-col items-center mb-8 border border-purple-100 shadow-inner h-48 justify-end relative overflow-hidden">
              <PixelVocabmon level={devEvoModal.to} className="scale-125 origin-bottom relative z-10" />
            </div>
            <button onClick={() => setDevEvoModal(null)} className="w-full bg-purple-600 hover:bg-purple-700 text-white font-black py-4 px-4 rounded-xl text-xl transition-transform active:scale-95 shadow-md">
              Close Preview 🛠️
            </button>
          </div>
        </div>
